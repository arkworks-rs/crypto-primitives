...
#[derive(Clone)]
pub struct Sha512Gadget<ConstraintF: PrimeField> {
...
impl<ConstraintF: PrimeField> Default for Sha512Gadget<ConstraintF> {
    fn default() -> Self {
        Self {
...
impl<ConstraintF: PrimeField> Sha512Gadget<ConstraintF> {
    fn update_state(
    ...
    pub fn digest(data: &[UInt8<ConstraintF>]) -> Result<DigestVar<ConstraintF>, SynthesisError> {
        let mut sha512_var = Self::default();
        sha512_var.update(data)?;
        sha512_var.finalize()
    }
}

// Now implement the CRH traits for SHA512

/// Contains a 64-byte SHA512 digest
#[derive(Clone, Debug)]
pub struct DigestVar<ConstraintF: PrimeField>([UInt8<ConstraintF>; 64]);
...
impl<ConstraintF> CRHSchemeGadget<Sha512, ConstraintF> for Sha512Gadget<ConstraintF>
where
    ConstraintF: PrimeField,
{
...
impl<ConstraintF> TwoToOneCRHSchemeGadget<Sha512, ConstraintF> for Sha512Gadget<ConstraintF>
where
    ConstraintF: PrimeField,
{
    type InputVar = [UInt8<ConstraintF>];
    ...
    fn compress(
        ...
        <Self as TwoToOneCRHSchemeGadget<Sha512, ConstraintF>>::evaluate(
            parameters,
            &left_input,
            &right_input,
        )
...
mod test {
    ...
    /// Finalizes a SHA512 gadget and gets the bytes
    fn finalize_var(sha512_var: Sha512Gadget<Fr>) -> Vec<u8> {
        sha512_var.finalize().unwrap().value().unwrap().to_vec()
    }

    /// Finalizes a native SHA512 struct and gets the bytes
    fn finalize(sha512: Sha512) -> Vec<u8> {
        sha512.finalize().to_vec()
    }

    /// Tests the SHA512 of random strings of varied lengths
    #[test]
    fn varied_lengths() {
        ...
        for &len in TEST_LENGTHS {
            ...
            let mut sha512_var = Sha512Gadget::default();
            ...
            sha512_var
                ...
                .unwrap();
            sha512.update(input_str);
            assert_eq!(
                finalize_var(sha512_var),
                finalize(sha512),
                "error at length {}",
                len
            );
    ...
    fn many_updates() {
        ...
        assert_eq!(finalize_var(sha512_var), finalize(sha512));
    }
    ...
    fn crh() {
        ...
        for &len in TEST_LENGTHS {
            ...
            // Compute the hashes and assert consistency
            let computed_output = <Sha512Gadget<Fr> as CRHSchemeGadget<Sha512, Fr>>::evaluate(
                &unit_var,
                &to_byte_vars(ns!(cs, "input"), &input_str),
            )
            .unwrap();
            let expected_output = <Sha512 as CRHScheme>::evaluate(&unit, input_str).unwrap();
            assert_eq!(
    ...
    fn to_to_one_crh() {
        ...
        for &len in TEST_LENGTHS {
            ...
            // Compute the hashes and assert consistency
            let computed_output =
                <Sha512Gadget<Fr> as TwoToOneCRHSchemeGadget<Sha512, Fr>>::evaluate(
                    &unit_var,
                ...
                .unwrap();
            let expected_output =
                <Sha512 as TwoToOneCRHScheme>::evaluate(&unit, left_input, right_input).unwrap();
            assert_eq!(
                computed_output.value().unwrap().to_vec(),
                expected_output,
